...
/*
 ...
 (Pointer chỉ phục vụ cho 1 mục đích: sharing (chia sẻ) )

 Pointers share values across the program boundaries.
 (Pointers chia sẻ values qua các ranh giới của chương trình.)

 There are several types of program boundaries. The most common one is between     	function calls.
 (có vài kiểu program boundaries. Nhưng phổ biến vẫn là giữa các function calls.)

 We can also have a boundary between Goroutines which we will discuss later.
 (Chúng ta cũng có một boundary giữa các Goroutines, ta sẽ bàn luận sau.)

 When this program starts up, the runtime creates a Goroutine.
 (Khi chương trình bắt đầu, runtime sẽ tạo một goroutine.)

Every Goroutine is a separate path of execution that contains instructions that needed to be executed by the machine. We can also think of Goroutines as lightweight threads.
(Mỗi Goroutine là một path thực thi riêng biệt và mỗi path đó chữa instructions [lệnh chỉ thị] cần thực thi bởi machine. Chúng ta có thể nghĩ goroutines như một
lightweigth threads. )

This program has only 1 Goroutine: the main Goroutine.
(Chương trình này sẽ chỉ có 1 Goroutine: main Goroutine.)

Every Goroutine is given a block of memory, called the stack.
(Mỗi Goroutine có một block của memory, gọi là stack.)

The stack memory in Go starts out at 2K. It is very small. It can change over time.
(Stack memory trong Go bắt đầu với 2k. Nó rất nhỏ. Nó có thể thay đổi theo thời gian)

Every time a function is called, a piece of stack is used to help that function run.
(Mỗi khi một function được gọi, một phần của stack được sử dụng giúp cho function chạy.)

The growing direction of the stack is downward.
(Hướng phát triển của stack là downward [hạ xuống] )

Every function is given a stack frame, memory execution of a function.
(mỗi function có một stack frame, memory excution [ bộ nhớ thực thi ] của một function)
...
(Tại thời điểm biên dịch sẽ không biết kích thước của mỗi stack frame. Không giá trị nào có thể đặt trên stack, trừ khi trình biên dịch biết kích thước của nó.)


If we don't know the size of something at compile time, it has to be on the heap.
(Nếu không biết kích thước tại thời điểm compiler, nó có thể đặt trên heap.)
